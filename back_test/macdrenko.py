...
# date_column = "Time"
# file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'L:/data/mt5/OANDA-Japan MT5 Live/mt5_USDJPY_min30.csv'))
# frame = 30
file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'L:/data/mt5/OANDA-Japan MT5 Live/mt5_USDJPY_h1.csv'))
frame = 60
# file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'L:/data/mt5/OANDA-Japan MT5 Live/mt5_USDJPY_h2.csv'))
# frame = 120
# file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'L:/data/mt5/OANDA-Japan MT5 Live/mt5_USDJPY_d1.csv'))
# frame = 60*24
date_column = "time"
ohlc_columns = ["open", "high", "low", "close"]

def MACDRenkoByBBCSV(slope=5):
    client = CSVClient(files=file_path, auto_step_index=True,frame=frame, start_index=120, logger=logger, columns=ohlc_columns, date_column=date_column, slip_type="percent")
...
def MACDRenkoRangeSLCSV(slope_window=5, use_tp=True):
    client = CSVClient(files=file_path, auto_step_index=True, frame=frame, start_index=150, logger=logger, columns=ohlc_columns ,date_column=date_column, slip_type="percent", do_render=False)
    columns = client.get_ohlc_columns()
    macd_p = MACDProcess(short_window=12, long_window=26, signal_window=9, target_column=ohlc_columns[3])
    renko_p = RenkoProcess(window=60, ohlc_column=ohlc_columns)
    rtp_p = RangeTrendProcess(slope_window=3)
    bband_process = BBANDProcess(target_column=columns["Close"], alpha=2)
    st1 = ts.strategies.MACDRenkoRangeSLByBB(client, renko_p, macd_p, bband_process, rtp_p, slope_window=slope_window, interval_mins = 0, data_length=70, use_tp=use_tp, logger=logger)
