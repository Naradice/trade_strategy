    ...
    if position is None:
        if last_slope_value > slope_threshold and rsi_threshold > df[rsi_column].iloc[-1]:
            if trend_possibility > ema_threshold:
                signal = BuySignal(key, volume=volume, price=df[order_price_column].iloc[-1])
                in_range = False
            elif trend_possibility > -ema_threshold:
                signal = SellSignal(key, volume=volume, price=df[order_price_column].iloc[-1])
                in_range = True
...
def macd_cross(position, previouse_trend, data, target_column="Close", signal_column_name="Signal", macd_column_name="MACD"):
    ...
    signal = None
    if tick_trend != 0:
        long_short = position.position_side.value if position is not None else 0
        if previouse_trend == 1 and tick_trend == -1:
            previouse_trend = tick_trend
            if long_short == 1:
                signal = CloseSellSignal(key, price=price)
            else:
...
def range_experimental(
    ...
    if rp < 0.6:
        ...
        if long_short != 0:
            if long_short == 1 and tp > slope_ratio:
                pass
            elif long_short == -1 and tp < -slope_ratio:
                pass
            else:
                signal = CloseSignal(key, price=df[order_price_column].iloc[-1])
